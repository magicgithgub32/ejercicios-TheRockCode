...
useEffect(() => {
  setNewListOfMovies(wantedGenres.name === 'All' ? movies 
...
return (
  <div>
    <h1>Movies</h1>
    <select value={wantedGenres.id} onChange={event => setWantedGenres(genres.find(genre => genre.id === event.target.value))}>
      {genres.map(genre => (
